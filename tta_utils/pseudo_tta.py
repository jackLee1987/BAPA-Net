...
from .comm import is_main_process, get_world_size
from .comm import all_gather
from .comm import synchronize
from .timer import Timer, get_time_str
from .log_image_bb import log_test_image
from .bounding_box import BoxList
from .boxlist_iou import boxlist_iou
...
def compute_on_dataset(model, data_loader, device, tta_breakpoints, timer=None, cfg=None):
    ...
    for _, batch in enumerate(tqdm(data_loader)):
        ...
        assert len(batch) == 1, 'Must work with a batch size of 1 for TTA'
        images, targets, image_ids = batch[0]["image"], batch[0]["instances"], batch[0]["image_id"]
        ...
        with torch.no_grad():
            # output = model(images.to(device))
            output = model(batch)
            output_boxlist = BoxList((output[0]['instances']._fields)['pred_boxes'].tensor, (batch[0]['width'], batch[0]['height']))
            output_boxlist.add_field("scores",(output[0]['instances']._fields)['scores'])
            output_boxlist.add_field("labels",(output[0]['instances']._fields)['pred_classes'])
            output2iou = [output_boxlist]
            # output2iou = [BoxList((output[0]['instances']._fields)['pred_boxes'].tensor, (batch[0]['width'], batch[0]['height']))]
        
        # output_used = output
        output_used2iou = output2iou
        batch_temp = copy.deepcopy(batch)
        for tta_it in range(DEFAULT_ITERATIONS):     
            model.train()
            optimizer.zero_grad()
            ...
            imgs = torch.flip(imgs.unsqueeze(0),dims=(3,)).squeeze(0)
            batch_temp[0]["image"] = imgs
            with torch.no_grad():
                output = model(batch_temp)
                # output2iou = [BoxList((output[0]['instances']._fields)['pred_boxes'].tensor, (batch[0]['width'], batch[0]['height']))]
                output_boxlist = BoxList((output[0]['instances']._fields)['pred_boxes'].tensor, (batch[0]['width'], batch[0]['height']))
                output_boxlist.add_field("scores",(output[0]['instances']._fields)['scores'])
                output_boxlist.add_field("labels",(output[0]['instances']._fields)['pred_classes'])
                ...
                pseudo_output,_ = process_pseudo_label_confidence(output_used2iou,output2iou,1.0,0.6,device)
                pseudo_output,_ = process_pseudo_labels_steps(pseudo_output,device) 
                if pseudo_output == None:
                    model.eval()
                    with torch.no_grad():
                        output = model(batch)
                        prediction_item = {"image_id": batch[0]["image_id"]}
                        instances = output[0]["instances"].to("cpu")
                        prediction_item["instances"] = instances_to_coco_json(instances, batch[0]["image_id"])
                        predition_list.append(prediction_item)
                    break
            output_used2iou = output2iou
            # output_used = output
            
            model.train()
            model.turn_off_box_training(force=True)
            model.turn_off_cls_training(force=True)
            # model.only_train_mask = True
            (batch_temp[0]["instances"]._fields)['gt_boxes'] = Boxes(pseudo_output[0].bbox)
            (batch_temp[0]["instances"]._fields)['gt_classes'] = pseudo_output[0].extra_fields['labels']
            j_loss_dict = model(batch_temp)
            ...
            if tta_it == DEFAULT_ITERATIONS - 1:
                model.eval()
                with torch.no_grad():
                    output = model(batch)
                    prediction_item = {"image_id": batch[0]["image_id"]}
