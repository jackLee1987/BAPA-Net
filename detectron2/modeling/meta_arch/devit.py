...
import sys  # 1. 导入sys模块（放在代码开头或打印分布前）
import math
import random
import logging
import numpy as np
...
class ProtoCenterAttentionV2(nn.Module):
    def __init__(self, input_size, num_heads=4, dropout=0.1):
        super(ProtoCenterAttentionV2, self).__init__()
    ...
    def forward(self, x):
        ...
        return output, attn_scores[:, 0, :, :].mean(dim=1)

class ProtoCenterAttention(nn.Module):
    def __init__(self, input_size, reduction=2):
...
class InstReweight(nn.Module):
    ...
    def __init__(self, c_in,k_shot, ratio=0.5):
        super(InstReweight, self).__init__()
        if k_shot <=5:
            self.proto_center_attn = ProtoCenterAttentionV2(c_in).cuda()
        else:
...
class AttentionClassScoring(nn.Module):
    def __init__(self, feat_dim, num_classes, num_heads=4):
        ...
        self.roi_to_query = nn.Linear(feat_dim, feat_dim)
        self.class_to_key = nn.Linear(feat_dim, feat_dim)
        self.class_to_value = nn.Linear(feat_dim, feat_dim)
        self.multihead_attn = nn.MultiheadAttention(
            embed_dim=feat_dim,
            num_heads=num_heads,
            batch_first=True
        )
        self.scorer = nn.Linear(feat_dim, num_classes)

    def forward(self, roi_features, class_weights, return_attn=False):
        B, C, num_patches = roi_features.shape  #
        class_weights = class_weights.to(roi_features.device)

        roi_flat = roi_features.permute(0, 2, 1)  # (B, num_patches, C)

        # 2. (num_classes, C) → (B, num_classes, C) → Key/Value
        class_batch = class_weights.unsqueeze(0).expand(B, -1, -1)  # (B, num_classes, C)

        queries = self.roi_to_query(roi_flat)  # (B, num_patches, C) → Query=ROI
        keys = self.class_to_key(class_batch)  # (B, num_classes, C) → Key
        values = self.class_to_value(class_batch)  # (B, num_classes, C) → Value
        ...
        roi_global = attn_output.mean(dim=1)  # (B, C)

        scores = self.scorer(roi_global)  # (B, num_classes)
...
class OpenSetDetectorWithExamples(nn.Module):
    ...
    def optimal_transport_fuse_bg_prototypes(self, bg_prototypes_paths, beta=0.0, reg=0.2, max_iter=300,
                                             temp=0.5, domain_adapt_weight=0.4):
        """
        Cross-domain prototype fusion based on optimal transport: fine-tune COCO background prototypes with sufficient downstream scene background features
        """
        bg_prototypes_paths = bg_prototypes_paths.split(",")
        # Load old prototypes (COCO background) and new prototypes (downstream cross-domain scene)
        old_protos = torch.load(bg_prototypes_paths[0])  # (530, 1024)
        new_protos = torch.load(bg_prototypes_paths[1])
        if isinstance(new_protos, dict):
            new_protos = new_protos['prototypes'].float()  # (K, 1024)

        # Check dimension consistency
        if old_protos.shape[-1] != new_protos.shape[-1]:
            raise ValueError(f"Feature dimension mismatch: {old_protos.shape[-1]} vs {new_protos.shape[-1]}")

        # Handle case with no new prototypes
        if len(new_protos) == 0:
            print("No new prototypes detected, returning original COCO background prototypes")
            return old_protos

        # --------------------------
        # 1. Feature normalization and domain shift calculation
        # --------------------------
        old_norm = F.normalize(old_protos, dim=1)  # (530, 1024)
        new_norm = F.normalize(new_protos, dim=1)  # (K, 1024)

        # Calculate overall domain shift (for cross-domain cost adjustment)
        old_mean = old_protos.mean(dim=0, keepdim=True)  # (1, 1024)
        new_mean = new_protos.mean(dim=0, keepdim=True)  # (1, 1024)
        domain_shift = torch.norm(old_mean - new_mean, p=2)  # Degree of domain shift

        # --------------------------
        # 2. Cross-domain robust cost matrix
        # --------------------------
        cosine_sim = torch.mm(old_norm, new_norm.T)  # (530, K)
        cost_matrix = 1 - (cosine_sim / temp) + domain_adapt_weight * domain_shift * (1 - cosine_sim)
        cost_matrix = torch.clamp(cost_matrix, min=0)  # Ensure non-negative cost

        # --------------------------
        # 3. Improved Sinkhorn-Knopp algorithm
        # --------------------------
        def sinkhorn_knopp(a, b, M, reg, max_iter, tol=1e-4):
            m, n = M.shape
            device = M.device

            u = torch.log(torch.ones(m, device=device) / m)
            v = torch.log(torch.ones(n, device=device) / n)
            K = torch.exp(-M / reg)  # Kernel matrix

            for i in range(max_iter):
                u_prev = u.clone()
                v = torch.log(b + 1e-8) - torch.logsumexp(u.unsqueeze(1) + torch.log(K + 1e-8), dim=0)
                u = torch.log(a + 1e-8) - torch.logsumexp(v.unsqueeze(0) + torch.log(K + 1e-8), dim=1)

                if torch.norm(u - u_prev, p=1) < tol:
                    print(f"Sinkhorn algorithm converged early, iteration count: {i + 1}")
                    break

            gamma = torch.exp(u.unsqueeze(1) + v.unsqueeze(0) + torch.log(K + 1e-8))
            return gamma

        # Define marginal distributions
        m = old_protos.shape[0]  # 530
        n = new_protos.shape[0]  # K
        a = torch.ones(m, device=old_protos.device) / m
        b = torch.ones(n, device=old_protos.device) / n

        # Calculate optimal matching matrix
        gamma = sinkhorn_knopp(a, b, cost_matrix, reg, max_iter)  # (530, K)
        gamma = gamma / (gamma.sum(dim=1, keepdim=True) + 1e-8)  # Row normalization

        # --------------------------
        # 4. Feature importance weighted fusion
        # --------------------------
        new_proto_var = torch.var(new_protos, dim=0, keepdim=True)  # (1, 1024)
        importance_weight = F.softmax(new_proto_var * 10, dim=1)  # Amplify differences

        new_contrib = torch.matmul(gamma, new_protos)  # (530, 1024)
        new_contrib = new_contrib * importance_weight  # Apply importance weight

        # --------------------------
        # 5. Adaptive fine-tuning update
        # --------------------------
        # Basic fusion
        updated_old_protos = (1 - beta) * old_protos + beta * new_contrib

        # --------------------------
        # 6. Result statistics and saving
        # --------------------------
        avg_new_influence = gamma.sum(dim=0).mean().item()
        print(f"beta={beta}")
        print(f"reg=：{reg}")
        print(f"domain_adapt_weight=：{domain_adapt_weight}")
        print(f"temp=：{temp}")
        save_path = "weights/background/background_prototypes.coco_fused.pth"
        torch.save(updated_old_protos, save_path)
        print(f"Cross-domain fused background prototypes saved to: {save_path}")

        return updated_old_protos
    ...
    def __init__(self,
        ...
        #add jack
        self.datasetname  = class_prototypes_file.split('/')[-1].split('_')[0]
        self.kshot= class_prototypes_file.split('/')[-1].split('_')[1].split('.')[0]
        ...
        if len(prototypes.shape) == 3:
            if self.set_cd_vito:
                ...
                self.register_buffer("test_class_weight", F.normalize(prototypes.mean(dim=1)[torch.as_tensor(test_class_order)], dim=-1))
                self.adapter = InstReweight(prototypes.shape[-1], self.k_shot,ratio=attn_fuse_ratio)
        ...
        bg_protos = self.optimal_transport_fuse_bg_prototypes(bg_prototypes_file)
        if isinstance(bg_protos, dict):  # NOTE: connect to dict output of `generate_prototypes`
            bg_protos = bg_protos['prototypes'].float()
    ...
    def forward(self, batched_inputs: List[Dict[str, torch.Tensor]]):
        ...
        if (self.training and (not self.only_train_mask)):
            ...
            if sample_class_enabled:
                ...
                #init_scores = self.attention_scoring(roi_features, class_weights)
            ...
            bg_logits = self.bg_cnn(torch.cat([bg_cls_dist_emb, bg_dist_emb], dim=1)) #bg_cls_dist_emb
            ...
            if sample_class_enabled:
                ...
                cache_logits_topk = torch.gather(
                    cache_logits.unsqueeze(2),  #
                    dim=1,
                    index=class_indices.unsqueeze(2)  #
                ).squeeze(2)  #
            else:
                cache_logits_topk = cache_logits

            if isinstance(cls_logits, list):
                for i in range(len(cls_logits)):
                    cls_logits[i] = cls_logits[i] * (1-alpha) + cache_logits * alpha
            else:
                cls_logits = cls_logits * (1-alpha) + cache_logits_topk * alpha
        ...
        if self.training:
            ...
            if self.set_cd_vito and self.num_train_classes != 1:
                class_prototypes = F.normalize(self.class_weights.weight.view(-1, self.k_shot, self.ndim).mean(dim=1), dim=-1)
